...
namespace CocktailsApp.Domain.Clubs
{
    ...
    ///     <description><see langword="required"/> <see cref="Clubs.Visibility"/> of the club</description>
    /// </item>
    /// </list> 
    /// </remarks>
    public sealed class Club : AggregateRoot, IAggregateRoot
    {
        ...
        /// <list type="bullet">
        /// <item><see cref="Visibility.Public"/></item>
        /// <item><see cref="Visibility.Private"/></item>
        /// </list>
        /// </para>
        /// <para>
        /// Visibility can be changed using the <see cref="UpdateVisibility(Visibility, Guid)"/> method.
        /// </para>
        /// </remarks>
        public Visibility Visibility { get => _visibility; }
        ...
        ) : base(ownerId)
        {
            ...
            owner.AddRole(ownerRole, ownerId);
            _roles.Add(ownerRole);

            // Set Club characteristics (w/o permission checks nor events raise):
            // - Address
            // - Name
            // - Visibility
            SetAddress(address);
            ...
            // Raise club created events
            AddDomainEvent(new ClubCreatedEvent(Id, Name, Description, Visibility, Address, ImageId, ownerId));
            AddDomainEvent(new ClubRoleCreatedEvent(Id, ownerRole.Id, ownerRole.Name, ownerRole.IsOwnerRole, ownerId));
            AddDomainEvent(new ClubMemberAddedEvent(Id, owner.Id, owner.UserId, ownerId));
            AddDomainEvent(new ClubMemberRoleAddedEvent(Id, owner.Id, ownerRole.Id, ownerId));
        }
        ...
        /// </summary>
        /// <remarks>
        /// Only <paramref name="actorId"/> with the <see cref="PermissionType.AddCocktail"/> permission can perform this action.
        /// </remarks>
        ...
        public ClubCocktail AddCocktail(Guid cocktailId, Guid actorId)
        {
            ...
            ValidateMemberPermission(actorId, PermissionType.AddCocktail);
            ...
            // Raise Cocktail added event
            AddDomainEvent(new ClubCocktailAddedEvent(Id, cocktail.Id, cocktail.CocktailId, actorId));
        ...
        /// </summary>
        /// <remarks>
        /// Only <paramref name="actorId"/> with the <see cref="PermissionType.AddMember"/> permission can perform this action.
        /// </remarks>
        ...
        public ClubMember AddMember(Guid userId, Guid actorId)
        {
            // Check the user's permissions:
            // Raise an exception if the user doesn't have permission to perform the action.
            ValidateMemberPermission(actorId, PermissionType.AddMember);
            ...
            // Raise Member added event
            AddDomainEvent(new ClubMemberAddedEvent(Id, member.Id, member.UserId, actorId));

            return member;
        }

        /// <summary>
        /// Adds a <see cref="PermissionType"/> to a <see cref="ClubRole"/> of the <see cref="Club"/>.
        /// </summary>
        /// <remarks>
        /// Only <paramref name="actorId"/> with the <see cref="PermissionType.AddPermissionToRole"/> permission can perform this action.
        /// </remarks>
        ...
        /// </exception>
        public void AddPermissionToRole(Guid roleId, PermissionType permission, Guid actorId)
        {
            // Check the user's permissions:
            // Raise an exception if the user doesn't have permission to perform the action.
            ValidateMemberPermission(actorId, PermissionType.AddPermissionToRole);

            // Throw an exception if specified role does not exist in the club
            var role = GetRole(roleId);

            // Add the permission to the role
            role.AddPermission(permission, actorId);

            // State that the club changed
            Touch(actorId);

            // Raise Permission added event
            AddDomainEvent(new ClubRolePermissionAddedEvent(Id, role.Id, permission, actorId));
        }
        ...
        /// </summary>
        /// <remarks>
        /// Only <paramref name="actorId"/> with the <see cref="PermissionType.AddRoleToMember"/> permission can perform this action.
        /// </remarks>
        ...
        public void AddRoleToMember(Guid clubMemberId, Guid roleId, Guid actorId)
        {
            // Check the user's permissions:
            // Raise an exception if the user doesn't have permission to perform the action.
            ValidateMemberPermission(actorId, PermissionType.AddRoleToMember);
            ...
            // Add role to member
            member.AddRole(role, actorId);

            // State that the club changed
            Touch(actorId);

            // Raise Role added to member event
            AddDomainEvent(new ClubMemberRoleAddedEvent(Id, member.Id, role.Id, actorId));
        }
        ...
        /// </summary>
        /// <remarks>
        /// Only <paramref name="actorId"/> with the <see cref="PermissionType.CreateRole"/> permission can perform this action.
        /// </remarks>
        ...
        public ClubRole CreateRole(string roleName, Guid actorId)
        {
            // Check the user's permissions:
            // Raise an exception if the user doesn't have permission to perform the action.
            ValidateMemberPermission(actorId, PermissionType.CreateRole);
            ...
            // Raise Role created event
            AddDomainEvent(new ClubRoleCreatedEvent(Id, role.Id, role.Name, role.IsOwnerRole, actorId));
        ...
        public void DeleteClub(Guid actorId)
        {
            ...
            // Raise club deleted event
            AddDomainEvent(new ClubDeletedEvent(Id, actorId));
        }
        ...
        /// <remarks>
        /// <para>
        /// Only <paramref name="actorId"/> with the <see cref="PermissionType.DeleteRole"/> permission can perform this action.
        /// </para>
        /// <para>
        ...
        public void DeleteRole(Guid roleId, Guid actorId)
        {
            // Check the user's permissions:
            // Raise an exception if the user doesn't have permission to perform the action.
            ValidateMemberPermission(actorId, PermissionType.DeleteRole);
            ...
            foreach (var member in _members)
                if (member.Roles.Any(r => r.Id == roleId))
                    member.RemoveRole(roleId, actorId);
            ...
            // Raise role delete event
            AddDomainEvent(new ClubRoleDeletedEvent(Id, role.Id, actorId));
        }
        ...
        /// </summary>
        /// <remarks>
        /// Only <paramref name="actorId"/> with the <see cref="PermissionType.RemoveCocktail"/> permission can perform this action.
        /// </remarks>
        ...
        public void RemoveCocktail(Guid cocktailId, Guid actorId)
        {
            // Check the user's permissions:
            // Raise an exception if the user doesn't have permission to perform the action.
            ValidateMemberPermission(actorId, PermissionType.RemoveCocktail);
            ...
            // Raise cocktail removed event
            AddDomainEvent(new ClubCocktailRemovedEvent(Id, cocktail.Id, actorId));
        }
        ...
        /// </summary>
        /// <remarks>
        /// Only <paramref name="actorId"/> with the <see cref="PermissionType.RemoveMember"/> permission can perform this action.
        /// </remarks>
        ...
        public void RemoveMember(Guid clubMemberId, Guid actorId)
        {
            // Check the user's permissions:
            // Raise an exception if the user doesn't have permission to perform the action.
            ValidateMemberPermission(actorId, PermissionType.RemoveMember);
            ...
            // Raise memeber removed event
            AddDomainEvent(new ClubMemberRemovedEvent(Id, member.Id, actorId));
        }

        /// <summary>
        /// Removes a <see cref="PermissionType"/> from a <see cref="ClubRole"/> within the <see cref="Club"/>.
        /// </summary>
        /// <remarks>
        /// Only <paramref name="actorId"/> with the <see cref="PermissionType.RemovePermissionFromRole"/> permission can perform this action.
        /// </remarks>
        /// <param name="roleId">The <see cref="Entity.Id"/> of the role.</param>
        /// <param name="permision">The <see cref="PermissionType"/> to remove.</param>
        /// <param name="actorId">The <see cref="Entity.Id"/> or the <see cref="ClubMember.UserId"/> of the club member performing the action.</param>
        /// <exception cref="UnauthorizedAccessException">
        /// Thrown when the <paramref name="actorId"/> is not authorized to perform the action.
        /// </exception>
        ...
        /// Thrown when the <paramref name="roleId"/> is not in the club.
        /// </exception>
        public void RemovePermissionFromRole(Guid roleId, PermissionType permision, Guid actorId)
        {
            // Check the user's permissions:
            // Raise an exception if the user doesn't have permission to perform the action.
            ValidateMemberPermission(actorId, PermissionType.RemovePermissionFromRole);
            ...
            role.RemovePermission(permision, actorId);

            // State that the club changed
            Touch(actorId);

            // Raise permission removed event
            AddDomainEvent(new ClubRolePermissionRemovedEvent(Id, role.Id, permision, actorId));
        }
        ...
        /// </summary>
        /// <remarks>
        /// Only <paramref name="actorId"/> with the <see cref="PermissionType.RemoveRoleFromMember"/> permission can perform this action.
        /// </remarks>
        /// <param name="clubMemberId">The <see cref="Entity.Id"/> of the member.</param>
        /// <param name="roleId">The <see cref="Entity.Id"/> of the role.</param>
        /// <param name="actorId">The <see cref="Entity.Id"/> or the <see cref="ClubMember.UserId"/> of the club member performing the action.</param>
        ...
        public void RemoveRoleFromMember(Guid clubMemberId, Guid roleId, Guid actorId)
        {
            // Check the user's permissions:
            // Raise an exception if the user doesn't have permission to perform the action.
            ValidateMemberPermission(actorId, PermissionType.RemoveRoleFromMember);
            ...
            // Raise role removed from member event
            AddDomainEvent(new ClubMemberRoleRemovedEvent(Id, member.Id, role!.Id, actorId));
        }
        ...
        /// </summary>
        /// <remarks>
        /// Only <paramref name="actorId"/> with the <see cref="PermissionType.ChangeAddress"/> permission can perform this action.
        /// </remarks>
        ...
        )
        {
            // Check the user's permissions:
            // Raise an exception if the user doesn't have permission to perform the action.
            ValidateMemberPermission(actorId, PermissionType.ChangeAddress);
            ...
            // Raise Address changed event
            AddDomainEvent(new ClubAddressChangedEvent(Id, Address!, actorId));
        }
        ...
        /// </summary>
        /// <remarks>
        /// Only <paramref name="actorId"/> with the <see cref="PermissionType.ChangeDescription"/> permission can perform this action.
        /// </remarks>
        ...
        public void UpdateDescription(string? newDescription, Guid actorId)
        {
            // Check the user's permissions:
            // Raise an exception if the user doesn't have permission to perform the action.
            ValidateMemberPermission(actorId, PermissionType.ChangeDescription);
            ...
            // Raise club description changed event
            AddDomainEvent(new ClubDescriptionChangedEvent(Id, Description, actorId));
        }
        ...
        /// </summary>
        /// <remarks>
        /// Only <paramref name="actorId"/> with the <see cref="PermissionType.ChangeName"/> permission can perform this action.
        /// </remarks>
        ...
        public void UpdateName(string? newName, Guid actorId)
        {
            // Check the user's permissions:
            // Raise an exception if the user doesn't have permission to perform the action.
            ValidateMemberPermission(actorId, PermissionType.ChangeName);
            ...
            // Raise club renamed event
            AddDomainEvent(new ClubRenamedEvent(Id, Name, actorId));
        }
        ...
        /// </summary>
        /// <remarks>
        /// Only <paramref name="actorId"/> with the <see cref="PermissionType.ChangeRoleName"/> permission can perform this action.
        /// </remarks>
        ...
        public void UpdateRoleName(string newName, Guid roleId, Guid actorId)
        {
            // Check the user's permissions:
            // Raise an exception if the user doesn't have permission to perform the action.
            ValidateMemberPermission(actorId, PermissionType.ChangeRoleName);

            // Throw an exception if the member does not exist in the club
            var role = GetRole(roleId);
            role.UpdateName(newName, actorId);

            // State that the club changed
            Touch(actorId);

            // Raise role renamed event
            AddDomainEvent(new ClubRoleRenamedEvent(Id, role.Id, role.Name, actorId));
        }
        ...
        /// </summary>
        /// <remarks>
        /// Only <paramref name="actorId"/> with the <see cref="PermissionType.ChangeVisibility"/> permission can perform this action.
        /// </remarks>
        ...
        /// Thrown when the <paramref name="actorId"/> is not authorized to perform the action.
        /// </exception>
        public void UpdateVisibility(Visibility visibility, Guid actorId)
        {
            // Check the user's permissions:
            // Raise an exception if the user doesn't have permission to perform the action.
            ValidateMemberPermission(actorId, PermissionType.ChangeVisibility);
            ...
            // Raise club visibility changed event
            AddDomainEvent(new ClubVisibilityChangedEvent(Id, Visibility, actorId));
        }
        ...
        {
            var cocktail = _cocktails.FirstOrDefault(cc => new ClubCocktailByIdSpecification(cocktailId).IsSatisfiedBy(cc))
        ...
        /// Thrown when the <paramref name="actorId"/> is not authorized to perform the action.
        /// </exception>
        private void ValidateMemberPermission(Guid actorId, PermissionType permission)
        {
        ...
        /// </returns>
        private bool IsMemberAuthorized(Guid actorId, PermissionType permission)
        {
