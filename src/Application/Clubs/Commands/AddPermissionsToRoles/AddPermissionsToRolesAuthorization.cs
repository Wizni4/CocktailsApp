...
{
    public sealed class AddPermissionsToRolesAuthorization(
        IClubAccess access
    ) : IAuthorize<AddPermissionsToRolesCommand>
    {
        private readonly IClubAccess _access = access;
        public async Task AuthorizeAsync(AddPermissionsToRolesCommand request, ICurrentUserService user, CancellationToken cancellationToken)
        {
