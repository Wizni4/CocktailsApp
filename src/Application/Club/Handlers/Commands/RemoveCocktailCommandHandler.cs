...
{
    /// <summary>
    /// Handles the <see cref="RemoveCocktailCommand"/> by removing a cocktail from a club.
    /// </summary>
    ...
    public class RemoveCocktailCommandHandler(IUnitOfWork unitOfWork, IMapper autoMapper)
        : ClubCommandHandler<RemoveCocktailCommand>(unitOfWork, autoMapper), ICommandHandler<RemoveCocktailCommand, ClubDTO>
    {
        ...
        /// </exception>
        public override async Task<ClubDTO> Handle(RemoveCocktailCommand request, CancellationToken cancellationToken)
        {
            ...
            club.RemoveCocktail(request.CocktailId, request.ClubId);
