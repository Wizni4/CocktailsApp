...
{
    /// <summary>
    /// Handles the <see cref="AddRoleToMemberCommand"/> by delegating to the club aggregate 
    ...
    public class AddRoleToMemberCommandHandler(IUnitOfWork unitOfWork, IMapper autoMapper)
        : ClubCommandHandler<AddRoleToMemberCommand>(unitOfWork, autoMapper), ICommandHandler<AddRoleToMemberCommand, ClubDTO>
    {
        ...
        /// </exception>
        public override async Task<ClubDTO> Handle(AddRoleToMemberCommand request, CancellationToken cancellationToken)
        {
            ...
            club.AddRoleToMember(request.MemberId, request.RoleId, request.ActorId);
