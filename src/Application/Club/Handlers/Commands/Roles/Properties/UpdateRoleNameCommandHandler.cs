...
{
    /// <summary>
    /// Handles the <see cref="UpdateRoleNameCommand"/> by deleting a role from a club.
    /// </summary>
    ...
    public class UpdateRoleNameCommandHandler(IUnitOfWork unitOfWork, IMapper autoMapper)
        : ClubCommandHandler<UpdateRoleNameCommand>(unitOfWork, autoMapper), ICommandHandler<UpdateRoleNameCommand, ClubDTO>
    {
        ...
        /// </exception>
        public override async Task<ClubDTO> Handle(UpdateRoleNameCommand request, CancellationToken cancellationToken)
        {
            ...
            club.UpdateRoleName(request.NewName, request.RoleId, request.ActorId);
