...
{
    /// <summary>
    /// Handles the <see cref="AddPermissionsToRoleCommand"/> by delegating to the club aggregate 
    ...
    public class AddPermissionsToRoleCommandHandler(IUnitOfWork unitOfWork, IMapper autoMapper)
        : ClubCommandHandler<AddPermissionsToRoleCommand>(unitOfWork, autoMapper), ICommandHandler<AddPermissionsToRoleCommand, ClubDTO>
    {
        ...
        /// </exception>
        public override async Task<ClubDTO> Handle(AddPermissionsToRoleCommand request, CancellationToken cancellationToken)
        {
            ...
            club.AddPermissionsToRole(request.RoleId, request.Permissions, request.ActorId);
