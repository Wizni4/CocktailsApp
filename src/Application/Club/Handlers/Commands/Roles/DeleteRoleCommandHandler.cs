...
{
    /// <summary>
    /// Handles the <see cref="DeleteRoleCommand"/> by deleting a role from a club.
    /// </summary>
    ...
    public class DeleteRoleCommandHandler(IUnitOfWork unitOfWork, IMapper autoMapper)
        : ClubCommandHandler<DeleteRoleCommand>(unitOfWork, autoMapper), ICommandHandler<DeleteRoleCommand, ClubDTO>
    {
        ...
        /// </exception>
        public override async Task<ClubDTO> Handle(DeleteRoleCommand request, CancellationToken cancellationToken)
        {
            // Load the club aggregate, including roles.
            var club = await base.GetClubFromRepositoryAsync(request.ClubId);

            // Delegate the club
            club.DeleteRole(request.RoleId, request.ActorId);
