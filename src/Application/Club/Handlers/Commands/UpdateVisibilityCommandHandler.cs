...
{
    /// <summary>
    /// Handles the <see cref="UpdateVisibilityCommand"/> by deleting a role from a club.
    /// </summary>
    ...
    public class UpdateVisibilityCommandHandler(IUnitOfWork unitOfWork, IMapper autoMapper)
        : ClubCommandHandler<UpdateVisibilityCommand>(unitOfWork, autoMapper), ICommandHandler<UpdateVisibilityCommand, ClubDTO>
    {
        ...
        /// </exception>
        public override async Task<ClubDTO> Handle(UpdateVisibilityCommand request, CancellationToken cancellationToken)
        {
            ...
            club.UpdateVisibility(request.Visibility, request.ActorId);
