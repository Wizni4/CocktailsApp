...
{
    /// <summary>
    /// Handles the <see cref="RemoveRoleToMemberCommand"/> by removing a role from a club member.
    /// </summary>
    ...
    public class RemoveRoleToMemberCommandHandler(IUnitOfWork unitOfWork, IMapper autoMapper)
        : ClubCommandHandler<RemoveRoleToMemberCommand>(unitOfWork, autoMapper), ICommandHandler<RemoveRoleToMemberCommand, ClubDTO>
    {
        ...
        /// </exception>
        public override async Task<ClubDTO> Handle(RemoveRoleToMemberCommand request, CancellationToken cancellationToken)
        {
            ...
            club.RemoveRoleToMember(request.MemberId, request.RoleId, request.ActorId);
