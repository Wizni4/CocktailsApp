...
{
    /// <summary>
    /// Handles the <see cref="UpdateOwnerCommand"/> by deleting a role from a club.
    /// </summary>
    ...
    public class UpdateOwnerCommandHandler(IUnitOfWork unitOfWork, IMapper autoMapper)
        : ClubCommandHandler<UpdateOwnerCommand>(unitOfWork, autoMapper), ICommandHandler<UpdateOwnerCommand, ClubDTO>
    {
        ...
        /// </exception>
        public override async Task<ClubDTO> Handle(UpdateOwnerCommand request, CancellationToken cancellationToken)
        {
            ...
            club.UpdateOwner(request.NewOwnerId, request.ActorId);
