...
{
    /// <summary>
    /// Handles the <see cref="RemoveRolePermissionCommand"/> by removing a permission from a club role.
    /// </summary>
    ...
    public class RemoveRolePermissionCommandHandler(IUnitOfWork unitOfWork, IMapper autoMapper)
        : ClubCommandHandler<RemoveRolePermissionCommand>(unitOfWork, autoMapper), ICommandHandler<RemoveRolePermissionCommand, ClubDTO>
    {
        ...
        /// </exception>
        public override async Task<ClubDTO> Handle(RemoveRolePermissionCommand request, CancellationToken cancellationToken)
        {
            ...
            club.RemoveRolePermission(request.RoleId, request.Permission, request.ActorId);
