...
{
    /// <summary>
    /// Handles the <see cref="RemoveRolesToMemberCommand"/> by removing a role from a club member.
    /// </summary>
    ...
    public class RemoveRolesToMemberCommandHandler(IUnitOfWork unitOfWork, IMapper autoMapper)
        : ClubCommandHandler<RemoveRolesToMemberCommand>(unitOfWork, autoMapper), ICommandHandler<RemoveRolesToMemberCommand, ClubDTO>
    {
        ...
        /// </exception>
        public override async Task<ClubDTO> Handle(RemoveRolesToMemberCommand request, CancellationToken cancellationToken)
        {
            ...
            club.RemoveRolesToMember(request.MemberId, request.RoleIds, request.ActorId);
